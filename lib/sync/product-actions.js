...
function _buildVariantImagesAction (diffedImages, oldVariant, newVariant) {
  ...
  // Apparently the diffpatch doesn't detect array moves
  // for images, causing unnecessary actions to be built.
  // This will check that add / remove actions for the same
  // image URL are filtered out.
  const actionsToBeRemovedByIndex = []
  forEach(actions, (action, i) => {
    if (action.action === 'addExternalImage') {
      const oppositeIndex = findIndex(actions, a =>
        a.action === 'removeImage' && a.imageUrl === action.image.url)
      if (oppositeIndex >= 0) {
        actionsToBeRemovedByIndex.push(i)
        actionsToBeRemovedByIndex.push(oppositeIndex)
      }
    }
  })
  // Safely remove (splice) from right -> left
  for (let i = actionsToBeRemovedByIndex.length - 1; i >= 0; i--)
    actions.splice(actionsToBeRemovedByIndex[i], 1)
    ...
    if (REGEX_NUMBER.test(key)) {
      ...
      if (Array.isArray(price) && price.length)
        ...
        addPriceActions.push({
          action: 'addPrice',
          variantId: oldVariant.id,
          price: diffpatcher.getDeltaValue(price)
        })
