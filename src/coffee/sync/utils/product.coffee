...
class ProductUtils extends BaseUtils
  ...
  matchesById: (variant1, variant2) ->
    @matchesBy(variant1, variant2, 'id')

  matchesByKey: (variant1, variant2) ->
    @matchesBy(variant1, variant2, 'key')

  matchesBySku: (variant1, variant2) ->
    @matchesBy(variant1, variant2, 'sku')

  matchesBy: (variant1, variant2, attributeToMatchBy) ->
    variant1 and variant2 and not isNil(variant1[attributeToMatchBy]) and variant1[attributeToMatchBy] == variant2[attributeToMatchBy]
